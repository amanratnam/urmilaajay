...
/**
 * InfiniteMenu — a draggable sphere of photographs (react-bits, vendored).
 * Adapted for this memorial: TypeScript, taint-free textures (photos are
 * served same-origin via /api/img so the atlas canvas never taints), each
 * cell cover-cropped to a clean square, a proper unmount/dispose path
 * (rAF cancel + WebGL context release) for HMR and route changes, and an
 * `onSelect` callback (fired when the front disc is chosen) instead of the
 * stock "open link" behaviour.
 */
...
type BufLocPair = [WebGLBuffer, number, number];

function makeVertexArray(
  gl: WebGL2RenderingContext,
  bufLocNumElmPairs: BufLocPair[],
  indices: Uint16Array
...
function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: ArrayBufferView | number, usage: number): WebGLBuffer {
  const buf = gl.createBuffer()!;
  gl.bindBuffer(gl.ARRAY_BUFFER, buf);
  // @ts-expect-error gl.bufferData accepts number | BufferSource across overloads
...
class ArcballControl {
  ...
  quatFromVectors(a: vec3, b: vec3, out: quat, angleFactor = 1): void {
...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    onActiveItemChange: (index: number) => void,
    onMovementChange: (moving: boolean) => void,
    onInit: (m: InfiniteGridMenu) => void,
    scale = 1.0
  ) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  dispose(): void {
    ...
    // Free GPU objects but keep the context intact — under React StrictMode
    // (and HMR) the same canvas is re-initialised right after unmount, and
    // losing the context would leave that remount with a dead GL context.
    const gl = this.gl;
    if (!gl) return;
    try {
      ...
      /* context already gone — nothing to free */
    }
  }

  private init(onInit: (m: InfiniteGridMenu) => void): void {
    const gl = this.canvas.getContext("webgl2", { antialias: true, alpha: true }) as WebGL2RenderingContext | null;
  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // Warm paper fill behind any image that failed to load.
        ctx.fillStyle = "#ECE7DE";
        ctx.fillRect(x, y, cellSize, cellSize);
        if (!img) return;
        // Cover-crop the photo into a clean centred square cell.
  ...
  private animate(deltaTime: number): void {
    ...
    positions.forEach((p, ndx) => {
      ...
      mat4.multiply(matrix, matrix, mat4.targetTo(mat4.create(), [0, 0, 0], p as number[], [0, 1, 0]));
  ...
  private render(): void {
    ...
    gl.uniformMatrix4fv(loc.uWorldMatrix as WebGLUniformLocation, false, this.worldMatrix);
    gl.uniformMatrix4fv(loc.uViewMatrix as WebGLUniformLocation, false, this.camera.matrices.view);
    gl.uniformMatrix4fv(loc.uProjectionMatrix as WebGLUniformLocation, false, this.camera.matrices.projection);
    gl.uniform3f(
    ...
    gl.uniform1i(loc.uItemCount as WebGLUniformLocation, this.items.length);
    gl.uniform1i(loc.uAtlasSize as WebGLUniformLocation, this.atlasSize);
    gl.uniform1f(loc.uFrames as WebGLUniformLocation, this.frames);
    gl.uniform1f(loc.uScaleFactor as WebGLUniformLocation, this.scaleFactor);
...
interface Props {
  items: MenuItem[];
  scale?: number;
  /** Active (front-facing) item index as the sphere settles. */
  onActiveIndex?: (index: number) => void;
  /** Fired when the visitor chooses the front photograph. */
  onSelect?: (index: number) => void;
}

export default function InfiniteMenu({ items, scale = 1.0, onActiveIndex, onSelect }: Props) {
  ...
  // A click that isn't a drag opens the front photograph.
  ...
  return (
    <canvas
      ...
      onPointerUp={(e) => {
        ...
        // A tap (little movement, quick) selects; a drag rotates the sphere.
        if (moved < 6 && Date.now() - d.t < 500) onSelect?.(activeIndexRef.current);
      }}
    />
  );
}
